    ...
    <div className="mx-auto w-full max-w-[1500px] px-4 pb-24 pt-5 sm:px-6 lg:px-8">
      <header className="flex flex-col gap-4 border-b border-slate-200 pb-5 lg:flex-row lg:items-end lg:justify-between">
        <div className="min-w-0">
          <button type="button" onClick={onBack} className="mb-3 inline-flex items-center gap-1.5 text-xs font-medium text-slate-500 transition hover:text-slate-900">
            <ArrowLeft className="size-3.5" /> Tipos de Eventos
          </button>
          <div className="flex flex-wrap items-center gap-2">
            <h1 className="text-2xl font-semibold tracking-tight text-slate-900">Configurar evento</h1>
            <span className={cn("inline-flex h-5 items-center gap-1.5 rounded-md px-2 text-xs font-semibold", draft.active ? "bg-emerald-50 text-emerald-700" : "bg-[#efedff] text-[#5a46d8]") }>
              <span className="size-1.5 rounded-full bg-current" /> {draft.active ? "Publicado" : "Em rascunho"}
            </span>
          </div>
          <p className="mt-1 max-w-xl text-sm leading-6 text-slate-500">Defina informações, regras de agendamento e acompanhe a experiência do cliente em tempo real.</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <label className="flex h-8 items-center gap-2 rounded-md border border-slate-200 bg-white px-2.5 text-xs font-medium text-slate-600">
            Ativo na página pública
            <Switch size="sm" checked={draft.active} onCheckedChange={(checked) => update("active", checked)} />
          </label>
          <Button type="button" variant="outline" size="sm" onClick={() => save("draft")} className="h-8 rounded-md bg-white px-3 text-xs shadow-none">{eventType ? "Salvar alterações" : "Salvar rascunho"}</Button>
          <Button type="button" size="sm" onClick={() => save("publish")} className="h-8 rounded-md bg-[#111827] px-3 text-xs shadow-none hover:bg-[#1f2937]"><Rocket className="size-3.5" />Publicar evento</Button>
        </div>
      </header>
        ...
        <div className="space-y-4">
          <EditorSection icon={FileText} title="Identificação do evento" aside="Etapa 1 de 4">
            ...
            <Field label="Link personalizado do evento">
              <div className="flex h-9 items-center rounded-md bg-[#f2f3fb] px-3 ring-1 ring-inset ring-slate-200 focus-within:ring-[#7b67e7]">
                <span className="hidden shrink-0 text-xs text-slate-400 sm:inline">{publicUrl.replace(/^https?:\/\//, "")}?tipo=</span>
                <input
                  value={draft.slug}
          ...
          <EditorSection icon={Clock3} title="Duração e janela de disponibilidade">
            <Field label="Duração da sessão">
              <div className="flex flex-wrap gap-1.5">
                {durationOptions.map((minutes) => (
                  <button key={minutes} type="button" onClick={() => { update("durationMinutes", minutes); setCustomDurationOpen(false); }} className={cn("h-8 rounded-md px-3 text-xs font-medium transition", draft.durationMinutes === minutes && !customDurationOpen ? "bg-[#111827] text-white" : "bg-[#f1f3fb] text-slate-600 hover:bg-[#e8ebf7]")}>{minutes} min</button>
                ))}
                <button type="button" onClick={() => setCustomDurationOpen(true)} className={cn("h-8 rounded-md px-3 text-xs font-medium transition", customDurationOpen ? "bg-[#111827] text-white" : "bg-[#f1f3fb] text-slate-600 hover:bg-[#e8ebf7]")}>Personalizado</button>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Abertura das reservas">
                <div className="flex items-center gap-2">
                  <button type="button" onClick={() => update("bookingWindowStart", "")} className={cn("h-9 shrink-0 rounded-md border px-3 text-xs font-medium", !draft.bookingWindowStart ? "border-[#7c68e8] bg-[#f0eeff] text-[#5d48dc]" : "border-slate-200 bg-white text-slate-500")}>Imediata</button>
            ...
            <Field label="Fuso horário base">
              <div className="mb-2 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <span className="text-xs text-slate-400">Horário usado para organizar as reservas.</span>
                <label className="flex items-center gap-2 text-xs font-medium text-[#5c48dc]"><Globe2 className="size-3.5" />Detectar automaticamente para o convidado <Switch size="sm" checked={draft.autoDetectTimezone} onCheckedChange={(checked) => update("autoDetectTimezone", checked)} /></label>
              </div>
          ...
          <EditorSection icon={Video} title="Canal e local da reunião">
            ...
            <div className="flex flex-col gap-3 rounded-lg bg-[#f3f4fb] p-3 sm:flex-row sm:items-center sm:justify-between">
              <div><p className="text-xs font-semibold text-slate-800">Valor do atendimento</p><p className="mt-0.5 text-xs text-slate-500">Deixe vazio para oferecer gratuitamente.</p></div>
              <div className="flex h-9 w-full items-center rounded-md border border-slate-200 bg-white px-3 sm:w-36"><span className="text-xs text-slate-400">R$</span><input type="number" min="0" step="0.01" value={draft.price ?? ""} onChange={(event) => update("price", event.target.value ? Number(event.target.value) : null)} placeholder="0,00" className="min-w-0 flex-1 bg-transparent text-right text-xs font-semibold text-slate-800 outline-none" /></div>
            </div>
          </EditorSection>
          ...
          <EditorSection icon={UsersRound} title="Participantes e lembretes automáticos">
            <Field label="Anfitriões adicionais / notificar por e-mail">
              <div className="flex min-h-10 flex-wrap items-center gap-1.5 rounded-md border border-slate-200 bg-[#f7f8fc] p-1.5 focus-within:border-[#8d7be7]">
                {draft.additionalHosts.map((email) => <span key={email} className="inline-flex h-7 items-center gap-1 rounded bg-white px-2 text-xs text-slate-600 ring-1 ring-slate-200">{email}<button type="button" onClick={() => update("additionalHosts", draft.additionalHosts.filter((item) => item !== email))} className="text-slate-400 hover:text-rose-500" aria-label={`Remover ${email}`}><X className="size-3" /></button></span>)}
          ...
          <EditorSection icon={Palette} title="Capa e personalização visual">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
              <div className="h-28 w-full overflow-hidden rounded-lg border border-slate-200 bg-gradient-to-br from-[#0f172a] via-[#334155] to-[#7c6be6] sm:w-44">
                {sanitizeCssUrl(draft.coverImage) && <div className="h-full w-full bg-cover bg-center" style={{ backgroundImage: `url(${JSON.stringify(sanitizeCssUrl(draft.coverImage))})` }} />}
              </div>
              <div className="min-w-0 flex-1"><p className="text-xs font-semibold text-slate-800">Banner do cabeçalho</p><p className="mt-1 text-xs leading-4 text-slate-500">Formato 16:9, mínimo 1200 × 675 px. PNG, JPG ou WebP até 5 MB.</p><div className="mt-3 flex flex-wrap gap-2"><input ref={fileInputRef} type="file" accept="image/png,image/jpeg,image/webp" className="hidden" onChange={(event) => uploadCover(event.target.files?.[0])} /><Button type="button" size="sm" onClick={() => fileInputRef.current?.click()} className="h-8 rounded-md bg-[#111827] px-3 text-xs shadow-none"><ImagePlus className="size-3.5" />{draft.coverImage ? "Alterar capa" : "Adicionar capa"}</Button><Button type="button" variant="secondary" size="sm" onClick={() => update("coverImage", "")} className="h-8 rounded-md px-3 text-xs shadow-none">Usar gradiente Weeki</Button></div></div>
            </div>
          </EditorSection>
        </div>

        <aside className="space-y-4 xl:sticky xl:top-4">
          <div className="flex items-center justify-between"><p className="flex items-center gap-1.5 text-xs font-semibold text-slate-700"><Globe2 className="size-3.5 text-[#5b47df]" />Pré-visualização da página pública</p><span className="inline-flex items-center gap-1 rounded bg-[#efedff] px-1.5 py-1 text-xs font-semibold text-[#5b47df]"><span className="size-1.5 rounded-full bg-[#6550e8]" />Em tempo real</span></div>
...
function VisibilityCard({ value, selected, title, description, onSelect }: { value: EventVisibility; selected: boolean; title: string; description: string; onSelect: () => void }) {
  return <button type="button" onClick={onSelect} className={cn("flex items-start gap-2.5 rounded-lg border p-3 text-left transition", selected ? "border-[#8a76eb] bg-[#f3f1ff]" : "border-transparent bg-[#f5f6fb] hover:border-slate-200")}><span className={cn("mt-0.5 grid size-4 shrink-0 place-items-center rounded-full border", selected ? "border-[#654fe4]" : "border-slate-300")}>{selected && <span className="size-2 rounded-full bg-[#654fe4]" />}</span><span><span className="flex items-center gap-1.5 text-xs font-semibold text-slate-800">{title}{value === "public" && <span className="rounded bg-[#6550e8] px-1 py-0.5 text-xs uppercase text-white">Recomendado</span>}</span><span className="mt-0.5 block text-xs leading-4 text-slate-500">{description}</span></span></button>;
}
...
function ChannelCard({ mode, current, title, description, icon: Icon, onSelect }: { mode: AppointmentMode; current: AppointmentMode; title: string; description: string; icon: typeof Video; onSelect: () => void }) {
  ...
  return <button type="button" onClick={onSelect} className={cn("min-h-28 rounded-lg border p-3 text-left transition", active ? "border-[#6955e8] bg-[#efedff] ring-1 ring-[#6955e8]" : "border-transparent bg-[#f3f4fb] hover:border-slate-200")}><span className="flex items-center justify-between"><span className={cn("grid size-8 place-items-center rounded-md", active ? "bg-[#5c48dd] text-white" : "bg-white text-slate-600")}><Icon className="size-4" /></span>{active && <Check className="size-3.5 text-[#5c48dd]" />}</span><span className="mt-3 block text-xs font-semibold text-slate-800">{title}</span><span className="mt-0.5 block text-xs leading-3 text-slate-500">{description}</span></button>;
}
...
function ReminderRow({ icon: Icon, title, description, checked, onChange }: { icon: typeof CalendarDays; title: string; description: string; checked: boolean; onChange: (checked: boolean) => void }) {
  return <label className="flex cursor-pointer items-center gap-3 rounded-lg bg-[#f3f4fb] p-3"><span className="grid size-8 shrink-0 place-items-center rounded-md bg-white text-[#5b47df]"><Icon className="size-4" /></span><span className="min-w-0 flex-1"><span className="block text-xs font-semibold text-slate-800">{title}</span><span className="mt-0.5 block text-xs leading-4 text-slate-500">{description}</span></span><input type="checkbox" checked={checked} onChange={(event) => onChange(event.target.checked)} className="size-4 accent-[#5b47df]" /></label>;
}
...
function PublicPreview({ draft, eventLink, days, selectedDate, selectedTime, onDateChange, onTimeChange }: { draft: EditorDraft; eventLink: string; days: Date[]; selectedDate: Date; selectedTime: string; onDateChange: (date: Date) => void; onTimeChange: (time: string) => void }) {
  return <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-[0_10px_25px_rgba(15,23,42,0.06)]"><div className="flex h-9 items-center gap-1.5 border-b border-slate-200 bg-[#f6f7fb] px-3"><span className="size-2 rounded-full bg-slate-300" /><span className="size-2 rounded-full bg-slate-300" /><span className="size-2 rounded-full bg-slate-300" /><div className="ml-2 flex h-6 min-w-0 flex-1 items-center gap-1.5 rounded bg-white px-2 text-xs text-slate-400"><Link2 className="size-2.5" /><span className="truncate">{eventLink.replace(/^https?:\/\//, "")}</span></div><button type="button" onClick={() => window.open(eventLink, "_blank")} className="grid size-6 place-items-center text-slate-400 hover:text-slate-700" aria-label="Abrir página pública"><ExternalLink className="size-3" /></button></div><div className="h-28 bg-gradient-to-br from-[#0f172a] via-[#334155] to-[#7c6be6] bg-cover bg-center" style={sanitizeCssUrl(draft.coverImage) ? { backgroundImage: `url(${JSON.stringify(sanitizeCssUrl(draft.coverImage))})` } : undefined}><div className="flex h-full items-start justify-end bg-gradient-to-t from-slate-950/25 to-transparent p-3">{draft.price !== null && draft.price > 0 && <span className="rounded-md bg-white/95 px-2 py-1 text-xs font-semibold text-slate-800">R$ {draft.price.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</span>}</div></div><div className="p-4"><div className="flex items-center gap-2"><span className="grid size-8 place-items-center rounded-full bg-[#111827] text-xs font-semibold text-white">WK</span><div><p className="text-xs font-semibold text-slate-800">Sua empresa</p><p className="text-xs text-slate-400">Atendimento profissional</p></div></div><h2 className="mt-4 text-base font-semibold tracking-[-0.025em] text-slate-900">{draft.name || "Título do seu evento"}</h2><div className="mt-2 flex flex-wrap gap-1.5"><PreviewChip icon={Clock3} label={formatDuration(draft.durationMinutes)} /><PreviewChip icon={draft.mode === "in_person" ? MapPin : draft.mode === "phone" ? Phone : Video} label={APPOINTMENT_MODE_LABELS[draft.mode]} /><PreviewChip icon={Globe2} label={draft.timezone === "America/Sao_Paulo" ? "GMT-3" : "Fuso local"} /></div><p className="mt-3 line-clamp-3 text-xs leading-4 text-slate-500">{stripHtml(draft.description) || "Adicione uma descrição objetiva para explicar como funciona o encontro e o que o cliente deve preparar."}</p></div><div className="border-t border-slate-100 p-4"><div className="mb-3 flex items-center justify-between"><p className="text-xs font-semibold text-slate-700">Selecione uma data e horário</p><span className="text-xs font-medium capitalize text-slate-500">{format(selectedDate, "MMMM yyyy", { locale: ptBR })}</span></div><div className="grid gap-3 sm:grid-cols-[1fr_92px]"><div><div className="grid grid-cols-7 text-center text-xs font-semibold text-slate-400">{["D", "S", "T", "Q", "Q", "S", "S"].map((day, index) => <span key={`${day}-${index}`}>{day}</span>)}</div><div className="mt-1 grid grid-cols-7 gap-y-0.5">{days.map((day) => <button key={day.toISOString()} type="button" onClick={() => onDateChange(day)} className={cn("grid aspect-square place-items-center rounded text-xs text-slate-600", !isSameMonth(day, selectedDate) && "text-slate-300", isSameDay(day, selectedDate) && "bg-[#111827] font-semibold text-white")}>{format(day, "d")}</button>)}</div></div><div className="space-y-1.5">{previewTimes.map((time) => <button key={time} type="button" onClick={() => onTimeChange(time)} className={cn("h-7 w-full rounded-md border text-xs font-medium", time === selectedTime ? "border-[#5b47df] bg-[#5b47df] text-white" : "border-slate-100 bg-slate-50 text-slate-700")}>{time}</button>)}</div></div><Button type="button" onClick={() => toast.info("Essa é uma pré-visualização. Publique o evento para receber reservas.")} className="mt-3 h-8 w-full rounded-md bg-[#111827] text-xs shadow-none">Confirmar e continuar <ChevronRight className="size-3.5" /></Button></div><div className="flex items-center justify-center gap-1.5 border-t border-slate-100 bg-[#fafbfc] py-3 text-xs text-slate-400"><CalendarDays className="size-3" />Agendamento protegido pela Weeki</div></div>;
}

function PreviewChip({ icon: Icon, label }: { icon: typeof Clock3; label: string }) {
  return <span className="inline-flex h-5 items-center gap-1 rounded bg-[#f0eeff] px-1.5 text-xs font-medium text-[#5a46d8]"><Icon className="size-2.5" />{label}</span>;
}
