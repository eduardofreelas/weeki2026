    ...
    <div className="mx-auto w-full max-w-[1500px] px-4 pb-24 pt-5 sm:px-6 lg:px-8">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-1.5 text-sm leading-6 text-slate-500">Receitas, despesas e previsões para decidir com mais clareza.</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          {view === "overview" && <Select value={rangeMonths} onValueChange={setRangeMonths}><SelectTrigger className="h-8 w-[142px] rounded-md bg-white text-xs shadow-none"><CalendarRange className="size-3.5" /><SelectValue /></SelectTrigger><SelectContent><SelectItem value="3">Últimos 3 meses</SelectItem><SelectItem value="6">Últimos 6 meses</SelectItem><SelectItem value="12">Últimos 12 meses</SelectItem></SelectContent></Select>}
          <Button type="button" variant="outline" size="sm" onClick={() => openCreate("expense")} className="h-8 rounded-md border-rose-200 bg-white px-2.5 text-xs text-rose-600 shadow-none hover:bg-rose-50"><ArrowUpRight className="size-3.5" /> Nova despesa</Button>
    ...
    <>
      ...
      <section className="mt-3 grid gap-3 xl:grid-cols-[minmax(0,1.65fr)_minmax(280px,0.75fr)]">
        <article className="min-w-0 rounded-xl border border-slate-200 bg-white p-4 sm:p-5">
          <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-sm font-semibold text-slate-900">Receita vs. despesas</h2><p className="mt-1 text-xs text-slate-400">Movimentações previstas por mês.</p></div><div className="flex items-center gap-3 text-xs text-slate-500"><span className="flex items-center gap-1.5"><span className="size-2 rounded-sm bg-[#6251e5]" />Receita</span><span className="flex items-center gap-1.5"><span className="size-2 rounded-sm bg-[#f08aa5]" />Despesas</span></div></div>
          <div className="mt-5 h-[260px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData} margin={{ top: 8, right: 4, left: -18, bottom: 0 }}>
                ...
                <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: "#94a3b8" }} tickFormatter={(value) => `${Math.round(value / 1000)}k`} />
                <Tooltip cursor={{ stroke: "#dfe3ea", strokeDasharray: "3 3" }} contentStyle={{ border: "1px solid #e2e8f0", borderRadius: 8, boxShadow: "none", fontSize: 13 }} formatter={(value, name) => [preciseCurrency.format(Number(value)), name === "receita" ? "Receita" : "Despesas"]} />
        ...
        <article className="rounded-xl border border-slate-200 bg-white p-4 sm:p-5">
          <div className="flex items-start justify-between"><div><h2 className="text-sm font-semibold text-slate-900">Previsão do mês</h2><p className="mt-1 text-xs text-slate-400">Receita confirmada e a receber.</p></div><span className="grid size-8 place-items-center rounded-md bg-[#f0edff] text-[#5d47df]"><Sparkles className="size-4" /></span></div>
          <div className="mt-6"><p className="text-xs font-semibold uppercase tracking-[0.06em] text-slate-400">Total previsto</p><p className="mt-1 text-2xl font-bold tracking-[-0.045em] text-slate-900">{currency.format(currentIncome + receivable)}</p></div>
  ...
  return (
    <>
      <div className="mt-5 flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
        ...
        <div className="flex gap-2"><Button type="button" variant="outline" size="sm" onClick={exportCsv} disabled={!filtered.length} className="h-8 rounded-md bg-white px-2.5 text-xs shadow-none"><Download className="size-3.5" /> Exportar CSV</Button><Button type="button" size="sm" onClick={onNew} className="h-8 rounded-md bg-[#5140df] px-3 text-xs shadow-none hover:bg-[#4432cf]"><Plus className="size-3.5" /> Nova transação</Button></div>
      </div>
      ...
      <div className="mt-3 grid gap-2 rounded-lg border border-slate-200 bg-white p-2 sm:grid-cols-2 xl:grid-cols-[minmax(210px,1.4fr)_150px_180px_190px_150px]">
        <div className="relative"><Search className="pointer-events-none absolute left-2.5 top-1/2 size-3.5 -translate-y-1/2 text-slate-400" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar no extrato..." className="h-8 rounded-md border-slate-200 pl-8 text-xs shadow-none" /></div>
        <Select value={period} onValueChange={(value) => setPeriod(value as PeriodFilter)}><SelectTrigger className="h-8 rounded-md text-xs shadow-none"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">Todo o período</SelectItem><SelectItem value="month">Mês atual</SelectItem><SelectItem value="quarter">Últimos 3 meses</SelectItem></SelectContent></Select>
...
function MetricCard({ label, value, previousValue, inverse = false, detail, tone, icon: Icon }: { label: string; value: number; previousValue?: number; inverse?: boolean; detail?: string; tone: "emerald" | "rose" | "violet" | "amber"; icon: typeof TrendingUp }) {
  ...
  return <article className="rounded-xl border border-slate-200 bg-white p-4"><div className="flex items-start justify-between"><p className="text-xs font-semibold uppercase tracking-[0.055em] text-slate-500">{label}</p><span className={cn("grid size-7 place-items-center rounded-md", tones[tone])}><Icon className="size-3.5" /></span></div><p className="mt-4 text-2xl font-bold tracking-[-0.04em] text-slate-900">{currency.format(value)}</p>{detail ? <p className="mt-1 text-xs text-slate-400">{detail}</p> : <p className={cn("mt-1 flex items-center gap-1 text-xs", positive ? "text-emerald-600" : "text-rose-500")}>{positive ? <TrendingUp className="size-3" /> : <TrendingDown className="size-3" />}{Math.abs(change).toFixed(1).replace(".", ",")}% <span className="text-slate-400">vs. mês anterior</span></p>}</article>;
}

function ForecastRow({ label, value, color }: { label: string; value: number; color: string }) { return <div className="flex items-center gap-2.5"><span className={cn("size-2 rounded-full", color)} /><span className="flex-1 text-xs text-slate-500">{label}</span><span className="text-xs font-semibold tabular-nums text-slate-800">{preciseCurrency.format(value)}</span></div>; }

function StatusBadge({ status }: { status: FinanceTransactionStatus }) {
  return <span className={cn("inline-flex h-6 items-center gap-1.5 rounded-md border px-2 text-xs font-medium", status === "paid" ? "border-emerald-200 bg-emerald-50 text-emerald-700" : status === "overdue" ? "border-rose-200 bg-rose-50 text-rose-700" : "border-amber-200 bg-amber-50 text-amber-700")}><span className="size-1.5 rounded-full bg-current" />{FINANCE_STATUS_LABELS[status]}</span>;
}

function EmptyFinance({ title, description }: { title: string; description: string }) {
  return <div className="grid min-h-36 place-items-center px-5 py-10 text-center"><div><ReceiptText className="mx-auto size-6 text-slate-300" /><h3 className="mt-3 text-xs font-semibold text-slate-700">{title}</h3><p className="mt-1 text-xs text-slate-400">{description}</p></div></div>;
}
