        ...
        <form onSubmit={submit}>
          <DialogHeader className="border-b border-slate-100 px-5 py-4 text-left sm:px-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <DialogDescription className="mt-1 text-xs">Registre a movimentação e mantenha seu fluxo atualizado.</DialogDescription>
              </div>
            </div>
          </DialogHeader>
          ...
          <div className="space-y-5 px-5 py-5 sm:px-6">
            ...
            <div className="rounded-lg border border-slate-200 bg-slate-50/70 px-4 py-3">
              <label htmlFor="finance-amount" className="text-xs font-semibold uppercase tracking-[0.07em] text-slate-500">Valor</label>
            ...
            <Field label="Situação">
              <div className="grid grid-cols-3 gap-1.5">
                {(["paid", "pending", "overdue"] as FinanceTransactionStatus[]).map((status) => <button key={status} type="button" onClick={() => update("status", status)} className={cn("h-8 rounded-md border border-slate-200 bg-white text-xs font-medium text-slate-500 transition", draft.status === status && (status === "paid" ? "border-emerald-200 bg-emerald-50 text-emerald-700" : status === "overdue" ? "border-rose-200 bg-rose-50 text-rose-700" : "border-amber-200 bg-amber-50 text-amber-700"))}>{FINANCE_STATUS_LABELS[status]}</button>)}
              </div>
            </Field>
            ...
            <section className="rounded-lg border border-slate-200 p-3.5">
              <div className="flex items-center gap-3"><span className="grid size-8 place-items-center rounded-md bg-[#f2efff] text-[#6048df]"><Repeat2 className="size-4" /></span><div className="min-w-0 flex-1"><p className="text-xs font-medium text-slate-800">Transação recorrente</p><p className="mt-0.5 text-xs text-slate-400">Use para mensalidades, assinaturas e custos fixos.</p></div><Switch size="sm" checked={draft.recurring} onCheckedChange={(checked) => update("recurring", checked)} /></div>
...
function Field({ label, children }: { label: string; children: ReactNode }) {
  return <label className="block"><span className="mb-1.5 block text-xs font-semibold uppercase tracking-[0.055em] text-slate-500">{label}</span>{children}</label>;
}
