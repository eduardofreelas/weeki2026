...
export function ClientsScreen({
  ...
  return (
    <div className="mx-auto flex w-full max-w-[1500px] flex-col px-4 py-5 sm:px-6 lg:px-8" style={{ minHeight: "calc(100vh - 4rem)" }}>
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div><h1 className="text-2xl font-semibold tracking-tight text-slate-900">Clientes</h1><p className="mt-1 text-sm text-slate-500">Centralize contatos, demandas, arquivos e contratos.</p></div>
        <div className="flex flex-1 flex-wrap items-center justify-end gap-2 sm:flex-none">
          <div className="relative min-w-[210px] flex-1 sm:w-[280px] sm:flex-none"><Search className="pointer-events-none absolute left-2.5 top-1/2 size-3.5 -translate-y-1/2 text-slate-400" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Pesquisar cliente..." aria-label="Pesquisar cliente" className="h-8 rounded-md bg-white pl-8 pr-9 text-sm shadow-none" /><kbd className="pointer-events-none absolute right-2.5 top-1/2 hidden -translate-y-1/2 rounded border border-slate-200 bg-slate-50 px-1 py-0.5 text-xs font-semibold text-slate-400 sm:inline">Ctrl K</kbd></div>
          <Button type="button" variant="outline" size="sm" onClick={() => setQuickOpen(true)}><UserRoundPlus className="size-3.5" /> <span className="hidden sm:inline">Cadastro rápido</span><span className="sm:hidden">Rápido</span></Button>
          <Button type="button" size="sm" onClick={() => { setSelectedClientId(null); setView("new"); }}><Plus className="size-3.5" /> Novo cliente</Button>
        </div>
      </div>

      <div className="mt-7 flex flex-wrap items-center justify-between gap-3">
        <div className="week-board-scroll flex max-w-full overflow-x-auto rounded-lg bg-slate-200/65 p-1">
          {statuses.map((item) => <button key={item.value} type="button" onClick={() => setStatus(item.value)} className={cn("focus-ring flex h-8 shrink-0 items-center gap-2 rounded-md px-3 text-xs font-medium text-slate-500 transition hover:text-slate-800", status === item.value && "bg-white font-semibold text-slate-800")}>{item.label}<span className={cn("rounded-full bg-slate-100 px-1.5 py-0.5 text-xs font-semibold text-slate-500", item.value === "active" && "bg-emerald-50 text-emerald-700", item.value === "negotiating" && "bg-amber-50 text-amber-700")}>{item.count}</span></button>)}
        </div>
        <div className="flex items-center gap-2">
          <div className="flex items-center rounded-lg border border-slate-200 bg-white p-0.5">
            <button type="button" onClick={() => setLayout("list")} className={cn("focus-ring grid size-8 place-items-center rounded-md text-slate-400 transition", layout === "list" && "bg-slate-100 text-slate-800")} aria-label="Visualização em lista"><List className="size-4" /></button>
            <button type="button" onClick={() => setLayout("grid")} className={cn("focus-ring grid size-8 place-items-center rounded-md text-slate-400 transition", layout === "grid" && "bg-slate-100 text-slate-800")} aria-label="Visualização em grade"><Grid2X2 className="size-4" /></button>
          </div>
          <DropdownMenu><DropdownMenuTrigger asChild><Button type="button" variant="outline" size="sm" className={cn("h-8 rounded-md bg-white px-2.5 text-xs shadow-none", demandFilter !== "all" && "border-[#afa1ed] bg-[#f5f3ff] text-[#6246db]")}><Filter className="size-3.5" /> Filtros</Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuCheckboxItem checked={demandFilter === "all"} onCheckedChange={() => setDemandFilter("all")}>Todos os clientes</DropdownMenuCheckboxItem><DropdownMenuCheckboxItem checked={demandFilter === "with_tasks"} onCheckedChange={() => setDemandFilter("with_tasks")}>Com demandas</DropdownMenuCheckboxItem><DropdownMenuCheckboxItem checked={demandFilter === "without_tasks"} onCheckedChange={() => setDemandFilter("without_tasks")}>Sem demandas</DropdownMenuCheckboxItem></DropdownMenuContent></DropdownMenu>
        </div>
      </div>

      {filteredClients.length ? layout === "list" ? (
        <>
          <div className="mt-4 grid gap-3 md:hidden">{filteredClients.map((client) => <ClientGridCard key={client.id} client={client} summary={taskSummary(client.id)} onOpen={() => openClient(client)} />)}</div>
          <div className="week-board-scroll mt-4 hidden overflow-x-auto rounded-xl border border-slate-200 bg-white md:block">
          ...
          <footer className="flex flex-wrap items-center justify-between gap-2 border-t border-slate-100 bg-slate-50/50 px-5 py-3 text-xs text-slate-400"><span>Mostrando {filteredClients.length} de {clients.length} clientes</span><span>Dados salvos neste navegador</span></footer>
        </div>
        </>
      ) : (
        ...
        <div className="mt-4 grid min-h-[320px] place-items-center rounded-xl border border-dashed border-slate-200 bg-white text-center"><div><Building2 className="mx-auto size-7 text-slate-300" /><h2 className="mt-3 text-sm font-semibold text-slate-700">Nenhum cliente encontrado</h2><p className="mt-1 text-xs text-slate-400">Ajuste os filtros ou cadastre um novo cliente.</p><Button type="button" size="sm" onClick={() => setView("new")} className="mt-4"><Plus className="size-3.5" /> Novo cliente</Button></div></div>
      )}
      ...
      <Dialog open={quickOpen} onOpenChange={setQuickOpen}>
        <DialogContent className="max-w-md gap-0 rounded-xl border-slate-200 p-0">
          <form onSubmit={saveQuickClient}>
            ...
            <div className="space-y-4 px-5 py-5"><div><Label htmlFor="quick-client-name" className="mb-1.5 block text-xs font-semibold text-slate-700">Nome ou razão social <span className="text-destructive">*</span></Label><Input id="quick-client-name" autoFocus value={quickDraft.name} onChange={(event) => setQuickDraft((current) => ({ ...current, name: event.target.value }))} placeholder="Ex.: Clínica Lumi" className="h-9" /></div><div><Label htmlFor="quick-client-contact" className="mb-1.5 block text-xs font-semibold text-slate-700">Responsável</Label><Input id="quick-client-contact" value={quickDraft.contactName} onChange={(event) => setQuickDraft((current) => ({ ...current, contactName: event.target.value }))} placeholder="Nome do contato" className="h-9" /></div><div className="grid grid-cols-2 gap-3"><div><Label htmlFor="quick-client-email" className="mb-1.5 block text-xs font-semibold text-slate-700">E-mail</Label><Input id="quick-client-email" type="email" value={quickDraft.email} onChange={(event) => setQuickDraft((current) => ({ ...current, email: event.target.value }))} placeholder="contato@..." className="h-9" /></div><div><Label htmlFor="quick-client-phone" className="mb-1.5 block text-xs font-semibold text-slate-700">Telefone</Label><Input id="quick-client-phone" type="tel" inputMode="numeric" value={quickDraft.phone} onChange={(event) => setQuickDraft((current) => ({ ...current, phone: formatPhoneBR(event.target.value) }))} placeholder="(00) 00000-0000" className="h-9" /></div></div></div>
            <DialogFooter className="flex-row border-t border-slate-100 bg-slate-50/60 px-5 py-3"><Button type="button" variant="ghost" size="sm" onClick={() => setQuickOpen(false)}>Cancelar</Button><Button type="submit" size="sm"><Check className="size-3.5" /> Salvar cliente</Button></DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
    </div>
  ...
  return (
    <div className="flex min-w-0 items-center gap-3">
      ...
      <div className="min-w-0">
        ...
        <p className="mt-1 truncate text-xs text-slate-400">{client.website || client.email || "Sem site informado"}</p>
      </div>
    </div>
  );
}
...
function ClientTableRow({ client, summary, onOpen }: { client: Client; summary: { all: Task[]; week: Task[]; completed: number }; onOpen: () => void }) {
  ...
  return <tr onClick={onOpen} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onOpen(); } }} tabIndex={0} className="group cursor-pointer transition hover:bg-slate-50/70 focus-visible:bg-slate-50"><td className="px-5 py-3"><ClientIdentity client={client} /></td><td className="px-4 py-3"><p className="flex items-center gap-1.5 text-xs text-slate-600"><span className="size-1.5 rounded-full bg-slate-300" />{client.contactName || "Sem responsável"}</p></td><td className="px-4 py-3"><span className={cn("inline-flex h-6 items-center gap-1.5 rounded-full px-2.5 text-xs font-semibold ring-1 ring-inset", statusStyles[client.status])}><span className="size-1.5 rounded-full bg-current" />{CLIENT_STATUS_LABELS[client.status]}</span></td><td className="px-4 py-3">{summary.week.length ? <div className="flex items-center gap-2.5"><div className="h-1.5 w-24 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-primary" style={{ width: `${progress}%` }} /></div><span className="text-xs tabular-nums text-slate-500">{summary.completed} de {summary.week.length} prontas</span></div> : <span className="text-xs italic text-slate-400">Nenhuma demanda na semana</span>}</td><td className="px-4 py-3 text-right"><span className="text-xs font-semibold tabular-nums text-slate-800">{client.contractValue ? currency.format(client.contractValue) : "R$ 0,00"}</span><span className="ml-2 rounded-md border border-slate-200 bg-slate-50 px-1.5 py-1 text-xs font-semibold uppercase text-slate-400">{client.contractKind === "fixed" ? "Fixo" : client.contractKind === "one_time" ? "Único" : "—"}</span></td><td className="px-3 py-3"><button type="button" onClick={(event) => { event.stopPropagation(); onOpen(); }} className="grid size-8 place-items-center rounded-md text-slate-300 opacity-0 transition hover:bg-slate-100 hover:text-slate-600 group-hover:opacity-100 focus:opacity-100" aria-label={`Abrir ${client.name}`}><MoreHorizontal className="size-4" /></button></td></tr>;
}

function ClientGridCard({ client, summary, onOpen }: { client: Client; summary: { all: Task[]; week: Task[]; completed: number }; onOpen: () => void }) {
  const progress = summary.week.length ? Math.round((summary.completed / summary.week.length) * 100) : 0;
  return <button type="button" onClick={onOpen} className="group rounded-xl border border-slate-200 bg-white p-4 text-left transition hover:border-[#c4b9f4] hover:bg-[#fdfcff]"><div className="flex items-start justify-between gap-3"><ClientIdentity client={client} /><ChevronRight className="size-4 text-slate-300 transition group-hover:translate-x-0.5 group-hover:text-[#6548df]" /></div><div className="mt-4 flex items-center justify-between"><span className={cn("inline-flex h-6 items-center gap-1.5 rounded-full px-2.5 text-xs font-semibold ring-1 ring-inset", statusStyles[client.status])}><span className="size-1.5 rounded-full bg-current" />{CLIENT_STATUS_LABELS[client.status]}</span><span className="text-xs font-semibold text-slate-800">{client.contractValue ? currency.format(client.contractValue) : "Sem contrato"}</span></div><div className="mt-4 border-t border-slate-100 pt-3"><div className="flex items-center justify-between text-xs text-slate-400"><span>{summary.week.length} {summary.week.length === 1 ? "demanda" : "demandas"} na semana</span><span>{progress}%</span></div><div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-[#6454e8]" style={{ width: `${progress}%` }} /></div>{client.email && <p className="mt-3 flex items-center gap-1.5 truncate text-xs text-slate-400"><Mail className="size-3" />{client.email}</p>}</div></button>;
}
