...
export function TaskSheet({
  ...
  const renderEstimate = (sectioned = false) => (
    <div>
      {sectioned ? <SectionTitle title="Tempo estimado" description="Quanto tempo essa demanda deve levar?" icon={Timer} /> : <FieldLabel>Tempo estimado</FieldLabel>}
      <div className="flex flex-wrap gap-1">
        {estimateOptions.map((option) => (
          <button
            ...
            className={cn(
              "focus-ring h-7 rounded-md border bg-white text-[11px] font-medium transition",
              sectioned ? "border-slate-200 px-2.5 text-slate-600 shadow-sm hover:border-slate-300 hover:bg-slate-50" : "px-2 text-slate-500 hover:border-[#a895f2] hover:text-[#6548df]",
        ...
        <button type="button" onClick={() => setCustomEstimateOpen(true)} className={cn("focus-ring h-7 rounded-md border bg-white text-[11px] font-medium transition", sectioned ? "border-slate-200 px-2.5 text-slate-600 shadow-sm hover:border-slate-300 hover:bg-slate-50" : "px-2 text-slate-500 hover:border-[#a895f2] hover:text-[#6548df]", customEstimateOpen && (sectioned ? "border-[#ddd6fe] bg-[#f5f3ff] font-semibold text-[#6d28d9]" : "border-[#8068e8] bg-[#f4f1ff] text-[#6548df]"))}>Personalizado</button>
      </div>
      {customEstimateOpen && (
        <div className="mt-2 flex items-center gap-2">
          <Input type="number" min={15} step={15} value={draft.estimateMinutes ?? ""} onChange={(event) => setDraft((current) => ({ ...current, estimateMinutes: event.target.value ? Number(event.target.value) : null }))} className="h-7 max-w-24 rounded-md bg-white px-2.5 text-xs" aria-label="Tempo personalizado em minutos" />
  ...
  const renderTags = (sectioned = false) => (
    <div>
      {sectioned ? <SectionTitle title="Etiquetas" description="Use marcadores curtos para organizar e localizar demandas." icon={Tags} /> : <FieldLabel>Tags</FieldLabel>}
      <Input
        value={tagInput}
    ...
    <form onSubmit={submit} className="flex h-full min-h-0 min-w-0 flex-1 flex-col">
          <SheetHeader className={cn("shrink-0 border-b border-slate-100 bg-white pr-14", task ? "px-5 pb-3.5 pt-4" : "px-5 py-4")}>
            <div className={cn("flex items-start", task ? "gap-3.5" : "gap-3")}>
              <div className={cn(
                "grid shrink-0 place-items-center rounded-lg text-[#694ce1] ring-1 ring-[#7657ff]/10",
            ...
            <div className="scrollbar-thin min-h-0 flex-1 overflow-y-auto px-5 py-4">
              <div className="space-y-5">
                <div>
                  ...
                  <Input autoFocus value={draft.title} onChange={(event) => setDraft((current) => ({ ...current, title: event.target.value }))} placeholder="Digite o título da demanda..." className="h-9 rounded-lg bg-white px-3 text-[13px] shadow-none focus-visible:border-[#7657ff] focus-visible:ring-2 focus-visible:ring-[#7657ff]/15" />
                </div>

                <div>
                  <FieldLabel>Cliente</FieldLabel>
                  <Select value={draft.clientId ?? "__none"} onValueChange={(value) => setDraft((current) => ({ ...current, clientId: value === "__none" ? null : value }))}>
                    <SelectTrigger className="h-9 w-full rounded-lg bg-white px-3 text-[13px] shadow-none"><SelectValue placeholder="Selecionar cliente" /></SelectTrigger>
                    <SelectContent>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-0.5 rounded-lg border border-slate-200/70 bg-slate-100/80 p-0.5">
                    <button type="button" onClick={() => { setDraft((current) => ({ ...current, scheduledDate: todayKey })); setShowDatePicker(false); }} className={cn("h-7 rounded-md px-2 text-[11px] font-medium text-slate-500 transition hover:text-slate-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#7657ff]/20", draft.scheduledDate === todayKey && !showDatePicker && "bg-white font-semibold text-[#6548df] shadow-sm")}>Hoje</button>
                    <button type="button" onClick={() => { setDraft((current) => ({ ...current, scheduledDate: tomorrowKey })); setShowDatePicker(false); }} className={cn("h-7 rounded-md px-2 text-[11px] font-medium text-slate-500 transition hover:text-slate-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#7657ff]/20", draft.scheduledDate === tomorrowKey && !showDatePicker && "bg-white font-semibold text-[#6548df] shadow-sm")}>Amanhã</button>
                    <button type="button" onClick={() => setShowDatePicker(true)} className={cn("h-7 rounded-md px-2 text-[11px] font-medium text-slate-500 transition hover:text-slate-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#7657ff]/20", showDatePicker && "bg-white font-semibold text-[#6548df] shadow-sm")}>Escolher data</button>
                  </div>
                ...
                <div>
                  ...
                  <Input type="time" value={draft.scheduledTime} onChange={(event) => setDraft((current) => ({ ...current, scheduledTime: event.target.value }))} className="h-9 rounded-lg bg-white px-3 text-[13px] shadow-none" />
                </div>
                  ...
                  <div className="space-y-5 border-t border-slate-100 pt-4">
                    ...
                    <div>
                      ...
                      <Select value={draft.priority} onValueChange={(value) => setDraft((current) => ({ ...current, priority: value as TaskPriority }))}>
                        <SelectTrigger className="h-9 w-full rounded-lg bg-white px-3 text-xs"><SelectValue /></SelectTrigger>
                    ...
                    <div className="grid gap-3 sm:grid-cols-2">
                      ...
                      <div><FieldLabel>Horário limite</FieldLabel><Input type="time" value={draft.dueTime} onChange={(event) => setDraft((current) => ({ ...current, dueTime: event.target.value }))} className="h-9 rounded-lg bg-white px-3 text-xs" /></div>
                    </div>
                    <div>
                      ...
                      {draft.recurrence.type === "custom" && (
                        <div className="mt-2.5 space-y-3 rounded-lg border border-slate-200 bg-white p-2.5">
                          <div>
            ...
            <Tabs defaultValue="details" className="min-h-0 flex-1 gap-0 overflow-hidden">
              <div className="scrollbar-thin min-h-0 flex-1 overflow-y-auto px-5 py-4">
                <div>
                  ...
                  <Input autoFocus value={draft.title} onChange={(event) => setDraft((current) => ({ ...current, title: event.target.value }))} placeholder="Título da demanda" className="h-10 rounded-lg border-[#9d8df0] bg-white px-3 text-[13px] font-medium text-slate-900 shadow-sm focus-visible:border-[#7c3aed] focus-visible:ring-2 focus-visible:ring-[#8b5cf6]/10" />
                </div>

                <div className="-mx-5 mt-3 border-b border-slate-200 px-5">
                  <TabsList variant="line" className="-mb-px flex h-10 w-full justify-start gap-3 rounded-none bg-transparent p-0 text-slate-500">
                    <TabsTrigger value="details" className="h-10 flex-none gap-1.5 rounded-none border-0 px-0.5 text-[11px] font-medium after:bottom-0 after:bg-[#7c3aed] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#8b5cf6]/20 data-[state=active]:font-semibold data-[state=active]:text-[#7c3aed]"><FileText className="size-3.5" />Detalhes</TabsTrigger>
                    <TabsTrigger value="checklist" className="h-10 flex-none gap-1.5 rounded-none border-0 px-0.5 text-[11px] font-medium after:bottom-0 after:bg-[#7c3aed] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#8b5cf6]/20 data-[state=active]:font-semibold data-[state=active]:text-[#7c3aed]"><CheckCircle2 className="size-3.5" />Checklist</TabsTrigger>
                    <TabsTrigger value="files" className="h-10 flex-none gap-1.5 rounded-none border-0 px-0.5 text-[11px] font-medium after:bottom-0 after:bg-[#7c3aed] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#8b5cf6]/20 data-[state=active]:font-semibold data-[state=active]:text-[#7c3aed]"><Paperclip className="size-3.5" />Anexos</TabsTrigger>
                    <TabsTrigger value="activity" className="h-10 flex-none gap-1.5 rounded-none border-0 px-0.5 text-[11px] font-medium after:bottom-0 after:bg-[#7c3aed] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#8b5cf6]/20 data-[state=active]:font-semibold data-[state=active]:text-[#7c3aed]"><History className="size-3.5" />Atividade</TabsTrigger>
                  </TabsList>
                </div>
                  ...
                  <TabsContent value="details" className="m-0 space-y-5 pt-4">
                    ...
                    <section>
                      ...
                      <div className="grid gap-3 sm:grid-cols-2">
                        ...
                        <div><ModalFieldLabel>Horário</ModalFieldLabel><Input type="time" value={draft.scheduledTime} onChange={(event) => setDraft((current) => ({ ...current, scheduledTime: event.target.value }))} className="h-9 rounded-lg border-slate-200 bg-white px-3 text-xs font-medium shadow-sm" /></div>
                      </div>
                    </section>
                    ...
                    <section>
                      ...
                      <div className="grid gap-3 sm:grid-cols-2">
                        ...
                        <div><ModalFieldLabel>Horário limite</ModalFieldLabel><Input type="time" value={draft.dueTime} onChange={(event) => setDraft((current) => ({ ...current, dueTime: event.target.value }))} className="h-9 rounded-lg border-slate-200 bg-white px-3 text-xs font-medium shadow-sm" /></div>
                      </div>
                    </section>
                    ...
                    <section>
                      ...
                      {draft.recurrence.type === "custom" && (
                        <div className="mt-2.5 space-y-3 rounded-lg border border-slate-200 bg-slate-50/50 p-2.5">
                          <div>
            ...
            {task ? (
              <>
                ...
                <div className="flex items-center gap-2 sm:gap-3">
                  ...
                  <Button type="button" variant="outline" size="sm" className="h-8 rounded-md border-slate-200 bg-white px-3 text-[11px] font-semibold text-slate-700 shadow-sm hover:bg-slate-100/80" onClick={() => onOpenChange(false)}>Cancelar</Button>
                  <Button type="submit" size="sm" className="h-8 rounded-md bg-[#7c3aed] px-3.5 text-[11px] font-semibold text-white shadow-sm hover:bg-[#6d28d9]">Salvar</Button>
                </div>
              </>
            ) : (
              <>
                ...
                <Button type="submit" size="sm" className="h-8 rounded-md bg-[#5b46e8] px-3 text-[11px] shadow-[0_4px_14px_rgba(105,78,226,0.16)] hover:bg-[#4f3bd5]"><Plus className="size-3.5" />Criar demanda</Button>
              </>
            )}
          </SheetFooter>
    </form>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent showCloseButton={false} className="flex max-h-[calc(100dvh-24px)] w-[calc(100vw-24px)] max-w-[600px] gap-0 overflow-hidden rounded-xl border border-slate-200/80 bg-white p-0 shadow-[0_24px_48px_-14px_rgba(15,23,42,0.2),0_0_0_1px_rgba(15,23,42,0.05)] sm:max-h-[90vh]">
        {panelContent}
      </DialogContent>
    </Dialog>
  );
