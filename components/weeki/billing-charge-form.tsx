...
export function BillingChargeForm({ charge, clients, settings, onBack, onSave }: { charge: BillingCharge | null; clients: Client[]; settings: BillingGatewaySettings; onBack: () => void; onSave: (draft: BillingChargeDraft, asDraft: boolean, id?: string) => void }) {
  ...
  return (
    <div className="mx-auto w-full max-w-[1500px] px-4 pb-24 pt-5 sm:px-6 lg:px-8">
      <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500"><button type="button" onClick={onBack} className="flex items-center gap-1.5 font-medium hover:text-slate-900"><ArrowLeft className="size-3.5" /> Cobranças</button><span className="text-slate-300">/</span><span className="font-semibold text-slate-800">{charge ? `Editar ${charge.code}` : "Nova cobrança"}</span><span className="rounded-md bg-slate-100 px-2 py-1 text-xs font-medium text-slate-500">Rascunho automático</span></div>
      <div className="mt-6"><h1 className="text-2xl font-semibold tracking-tight text-slate-900">Gerar cobrança por link</h1><p className="mt-1.5 text-sm leading-6 text-slate-500">Configure as condições e confira a experiência do pagador em tempo real.</p></div>

      {!settings.connected && <div className="mt-5 flex items-start gap-3 rounded-lg border border-amber-200 bg-amber-50 px-3.5 py-3 text-xs text-amber-800"><Sparkles className="mt-0.5 size-4 shrink-0" /><div><p className="font-semibold">Modo de demonstração</p><p className="mt-0.5 text-amber-700">Você pode montar e salvar cobranças. O link só aceitará pagamentos após a conexão segura com o Asaas.</p></div></div>}
        ...
        <div className="space-y-4">
          ...
          <SectionCard step="02" title="Opções de pagamento">
            ...
            <PaymentOption icon={CreditCard} title="Cartão de crédito" description="Parcelamento pelo checkout seguro do gateway." enabled={draft.methods.includes("credit_card")} onChange={(enabled) => toggleMethod("credit_card", enabled)}>
              {draft.methods.includes("credit_card") && <div className="mt-3 grid gap-3 border-t border-slate-200 pt-3 sm:grid-cols-2"><Field label="Parcelamento máximo"><Select value={String(draft.cardMaxInstallments)} onValueChange={(value) => update("cardMaxInstallments", Number(value))}><SelectTrigger className="h-8 rounded-md bg-white text-xs shadow-none"><SelectValue /></SelectTrigger><SelectContent>{[1,2,3,6,12].map((count) => <SelectItem key={count} value={String(count)}>Até {count}x</SelectItem>)}</SelectContent></Select></Field><Field label="Taxa de parcelamento"><div className="grid h-8 grid-cols-2 rounded-md bg-white p-0.5"><button type="button" onClick={() => update("passCardFees", true)} className={cn("rounded text-xs font-medium text-slate-500", draft.passCardFees && "bg-slate-900 text-white")}>Repassar</button><button type="button" onClick={() => update("passCardFees", false)} className={cn("rounded text-xs font-medium text-slate-500", !draft.passCardFees && "bg-slate-900 text-white")}>Assumir</button></div></Field></div>}
            </PaymentOption>
            ...
            <div className="rounded-lg border border-slate-200 p-3.5"><div className="flex items-center gap-3"><span className="grid size-8 place-items-center rounded-md bg-[#efedff] text-[#5c45df]">%</span><div className="min-w-0 flex-1"><p className="text-xs font-medium text-slate-800">Desconto condicional</p><p className="mt-0.5 text-xs text-slate-400">Incentive o pagamento por uma forma específica.</p></div><Switch size="sm" checked={draft.discountEnabled} onCheckedChange={(checked) => update("discountEnabled", checked)} /></div>{draft.discountEnabled && <div className="mt-3 grid gap-3 border-t border-slate-100 pt-3 sm:grid-cols-2"><Field label="Aplicar desconto em"><Select value={draft.discountMethod} onValueChange={(value) => update("discountMethod", value as BillingMethod)}><SelectTrigger className="h-8 rounded-md text-xs shadow-none"><SelectValue /></SelectTrigger><SelectContent>{draft.methods.map((method) => <SelectItem key={method} value={method}>{BILLING_METHOD_LABELS[method]}</SelectItem>)}</SelectContent></Select></Field><Field label="Percentual"><div className="relative"><Input type="number" min="0" max="100" step="0.1" value={draft.discountPercent} onChange={(event) => update("discountPercent", Number(event.target.value))} className="h-8 rounded-md pr-8 text-xs shadow-none" /><span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-slate-400">%</span></div></Field><div className="sm:col-span-2 flex items-center justify-between rounded-md bg-slate-50 px-3 py-2 text-xs"><span className="text-slate-500">Valor com desconto</span><strong className="font-semibold text-[#5b45df]">{currency.format(discountedAmount)}</strong></div></div>}</div>
          </SectionCard>
          ...
          <SectionCard step="03" title="Automação e comunicação">
            ...
            {draft.lateFeeEnabled && <div className="grid gap-3 rounded-lg bg-slate-50 p-3 sm:grid-cols-2"><Field label="Multa"><Input type="number" min="0" step="0.1" value={draft.lateFeePercent} onChange={(event) => update("lateFeePercent", Number(event.target.value))} className="h-8 rounded-md text-xs shadow-none" /></Field><Field label="Juros ao dia"><Input type="number" min="0" step="0.001" value={draft.dailyInterestPercent} onChange={(event) => update("dailyInterestPercent", Number(event.target.value))} className="h-8 rounded-md text-xs shadow-none" /></Field></div>}
          ...
          <div className="flex flex-col-reverse gap-2 rounded-xl border border-slate-200 bg-white p-3 sm:flex-row sm:items-center"><Button type="button" variant="ghost" size="sm" onClick={(event) => submit(event as unknown as FormEvent, true)} className="h-8 rounded-md text-xs text-slate-500">Salvar rascunho</Button><div className="sm:ml-auto flex gap-2"><Button type="button" variant="outline" size="sm" onClick={onBack} className="h-8 flex-1 rounded-md px-3 text-xs shadow-none sm:flex-none">Cancelar</Button><Button type="submit" size="sm" className="h-8 flex-1 rounded-md bg-[#111827] px-4 text-xs shadow-none hover:bg-[#272f3d] sm:flex-none"><Zap className="size-3.5" />{charge ? "Salvar cobrança" : "Gerar link de cobrança"}</Button></div></div>
        </div>
...
function BillingPreview({ draft, client, connected, discountedAmount }: { draft: BillingChargeDraft; client?: Client; connected: boolean; discountedAmount: number }) {
  return <aside className="xl:sticky xl:top-[88px]"><div className="overflow-hidden rounded-xl border border-slate-200 bg-white"><div className="flex h-10 items-center gap-1.5 bg-[#e9edff] px-3"><span className="size-2 rounded-full bg-slate-300" /><span className="size-2 rounded-full bg-slate-300" /><span className="size-2 rounded-full bg-slate-300" /><div className="mx-3 flex h-7 min-w-0 flex-1 items-center gap-2 rounded bg-white px-3 text-xs text-slate-500"><Link2 className="size-3 text-emerald-500" /><span className="truncate">weeki.com.br/pagar/previa</span></div><span className={cn("rounded-md px-2 py-1 text-xs font-semibold", connected ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700")}>{connected ? "Gateway ativo" : "Prévia"}</span></div><div className="bg-[#f7f8fc] p-4 sm:p-5"><div className="flex items-center gap-3"><span className="grid size-9 place-items-center rounded-lg bg-[#111827] text-xs font-semibold text-white">{client?.initials ?? "CL"}</span><div className="min-w-0"><p className="truncate text-xs font-semibold text-slate-900">Seu negócio</p><p className="truncate text-xs text-slate-400">Cobrança segura pela Weeki</p></div></div><div className="mt-4 rounded-lg border border-slate-200 bg-white p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.06em] text-slate-400">Pagador</p><p className="mt-1 text-xs font-semibold text-slate-800">{client?.name ?? "Cliente selecionado"}</p></div><div className="text-right"><p className="text-xs font-semibold uppercase tracking-[0.06em] text-slate-400">Vencimento</p><p className="mt-1 text-xs font-medium text-slate-700">{draft.dueDate ? format(parseISO(draft.dueDate), "dd MMM yyyy", { locale: ptBR }) : "—"}</p></div></div><div className="mt-4 rounded-md bg-[#f2f1ff] p-3"><p className="line-clamp-2 text-xs text-slate-500">{draft.description || "Descrição do serviço"}</p><div className="mt-2 flex items-end justify-between gap-3"><span className="text-xs font-medium text-slate-500">Total a pagar</span><strong className="text-xl font-bold tracking-[-0.04em] text-slate-900">{currency.format(draft.discountEnabled ? discountedAmount : draft.amount)}</strong></div>{draft.discountEnabled && <p className="mt-1 text-right text-xs text-[#5a45dc]">{draft.discountPercent}% de desconto via {BILLING_METHOD_LABELS[draft.discountMethod]}</p>}</div></div><div className="mt-3 grid grid-cols-3 gap-1 rounded-md bg-[#e8ebf8] p-1">{(["pix", "credit_card", "bank_slip"] as BillingMethod[]).map((method) => <span key={method} className={cn("flex h-7 items-center justify-center rounded text-xs font-medium text-slate-400", draft.methods.includes(method) && "bg-white text-slate-700")}>{BILLING_METHOD_LABELS[method]}</span>)}</div><div className="mt-3 grid min-h-[180px] place-items-center rounded-lg border border-slate-200 bg-white text-center"><div><QrCode className="mx-auto size-20 text-slate-900" strokeWidth={1.3} /><p className="mt-2 text-xs font-medium text-slate-600">Checkout exibido após integração</p><p className="mt-1 text-xs text-slate-400">Pix, cartão ou boleto conforme configuração</p></div></div>{draft.message && <div className="mt-3 rounded-md bg-[#eceefa] p-3"><p className="text-xs font-semibold uppercase text-slate-400">Mensagem</p><p className="mt-1 line-clamp-3 text-xs leading-4 text-slate-600">{draft.message}</p></div>}<p className="mt-4 flex items-center justify-center gap-1.5 text-xs text-slate-400"><Check className="size-3 text-emerald-500" /> Pagamento processado pelo gateway conectado</p></div></div><div className="mt-3 flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-xs text-slate-500"><span className="size-1.5 rounded-full bg-[#654ff0]" /><span className="flex-1">Prévia atualizada enquanto você edita</span><MessageCircle className="size-3.5" /></div></aside>;
}

function SectionCard({ step, title, children }: { step: string; title: string; children: ReactNode }) { return <section className="rounded-xl border border-slate-200 bg-white p-4 sm:p-5"><div className="mb-4 flex items-center"><span className="mr-2 size-2 rounded-full bg-[#6550eb]" /><h2 className="text-sm font-semibold text-slate-900">{title}</h2><span className="ml-auto text-xs font-semibold uppercase tracking-[0.06em] text-slate-400">Etapa {step}</span></div><div className="space-y-4">{children}</div></section>; }
function Field({ label, children }: { label: string; children: ReactNode }) { return <label className="block"><span className="mb-1.5 block text-xs font-semibold uppercase tracking-[0.05em] text-slate-500">{label}</span>{children}</label>; }
function PaymentOption({ icon: Icon, title, description, badge, enabled, onChange, children }: { icon: typeof CreditCard; title: string; description: string; badge?: string; enabled: boolean; onChange: (enabled: boolean) => void; children?: ReactNode }) { return <div className={cn("rounded-lg border p-3.5 transition", enabled ? "border-[#d9d4ff] bg-[#f5f3ff]" : "border-slate-200 bg-slate-50/70")}><div className="flex items-center gap-3"><span className={cn("grid size-8 place-items-center rounded-md", enabled ? "bg-white text-[#5a44dc]" : "bg-slate-100 text-slate-400")}><Icon className="size-4" /></span><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><p className="text-xs font-medium text-slate-800">{title}</p>{badge && <span className="rounded bg-slate-200 px-1.5 py-0.5 text-xs font-semibold uppercase text-slate-500">{badge}</span>}</div><p className="mt-0.5 text-xs text-slate-400">{description}</p></div><Switch size="sm" checked={enabled} onCheckedChange={onChange} /></div>{children}</div>; }
function ToggleRow({ title, description, checked, onChange }: { title: string; description: string; checked: boolean; onChange: (checked: boolean) => void }) { return <div className="flex items-center gap-3 rounded-lg bg-slate-50 p-3"><div className="min-w-0 flex-1"><p className="text-xs font-medium text-slate-800">{title}</p><p className="mt-0.5 text-xs text-slate-400">{description}</p></div><Switch size="sm" checked={checked} onCheckedChange={onChange} /></div>; }
