      ...
      <section className="overflow-hidden rounded-xl border border-slate-200 bg-white">
        <div className="grid gap-5 bg-gradient-to-r from-white via-white to-[#f2fffd] p-5 lg:grid-cols-[1fr_auto] lg:items-center">
          <div className="flex min-w-0 items-center gap-4">
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <h1 className="truncate text-2xl font-semibold tracking-tight text-slate-900">{client.name}</h1>
                <span className={cn("inline-flex h-5 items-center gap-1 rounded-full px-2 text-xs font-semibold ring-1 ring-inset", statusStyles[client.status])}><span className="size-1.5 rounded-full bg-current" />{CLIENT_STATUS_LABELS[client.status]}</span>
                <span className="rounded-md bg-slate-100 px-2 py-1 text-xs font-semibold text-slate-500">{CLIENT_KIND_LABELS[client.kind]}</span>
              </div>
          ...
          <div className="grid grid-cols-3 gap-2 rounded-xl bg-[#f2f3ff]/80 p-2">
            <div className="min-w-[112px] rounded-lg border border-slate-100 bg-white px-3 py-2.5"><span className="text-xs font-semibold uppercase tracking-wide text-slate-400">Contrato</span><p className="mt-1 text-sm font-semibold text-slate-900">{client.contractValue ? currency.format(client.contractValue) : "—"}<span className="text-xs font-normal text-slate-400">{client.contractKind === "fixed" ? "/mês" : ""}</span></p></div>
            <div className="min-w-[112px] rounded-lg border border-slate-100 bg-white px-3 py-2.5"><span className="text-xs font-semibold uppercase tracking-wide text-slate-400">Responsável</span><p className="mt-1 truncate text-sm font-semibold text-slate-900">{client.contactName || "—"}</p></div>
            <div className="min-w-[112px] rounded-lg border border-slate-100 bg-white px-3 py-2.5"><span className="text-xs font-semibold uppercase tracking-wide text-slate-400">Segmento</span><p className="mt-1 truncate text-sm font-semibold text-slate-900">{client.segment || "—"}</p></div>
          </div>
        </div>
      </section>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-[minmax(0,1fr)_300px]">
        <div className="min-w-0">
          <div className="week-board-scroll flex overflow-x-auto rounded-lg bg-[#eef0ff] p-1">
            {tabs.map((item) => <button key={item.value} type="button" onClick={() => setTab(item.value)} className={cn("focus-ring flex h-8 shrink-0 items-center gap-1.5 rounded-md px-3 text-xs font-medium text-slate-500 transition", tab === item.value && "bg-white font-semibold text-slate-800")}><item.icon className={cn("size-3.5", tab === item.value && "text-[#6650e4]")} />{item.label}{item.count !== undefined && <span className="rounded-full bg-[#eeeaff] px-1.5 py-0.5 text-xs font-semibold text-[#674bdd]">{item.count}</span>}</button>)}
          </div>

          {tab === "overview" && (
            <section className="mt-4 rounded-xl border border-slate-200 bg-white p-5">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div><h2 className="text-sm font-semibold text-slate-900">Demandas do cliente</h2><p className="mt-0.5 text-xs text-slate-400">Acompanhamento de entregas e pendências vinculadas.</p></div>
              ...
              <div className="mt-4 divide-y divide-slate-100 overflow-hidden rounded-lg border border-slate-100">
                {clientTasks.length ? clientTasks.map((task) => (
                  <article key={task.id} onClick={() => onOpenTask(task)} className="group flex cursor-pointer items-center gap-3 bg-white px-3 py-2.5 transition hover:bg-slate-50" role="button" tabIndex={0} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") onOpenTask(task); }}>
                    ...
                    <div className="min-w-0 flex-1"><h3 className={cn("truncate text-xs font-medium text-slate-800", task.status === "completed" && "text-slate-400 line-through")}>{task.title}</h3><div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-slate-400">{task.scheduledDate && <span className="flex items-center gap-1"><CalendarDays className="size-3" />{format(parseISO(task.scheduledDate), "EEE, dd/MM", { locale: ptBR })}{task.scheduledTime ? `, ${task.scheduledTime}` : ""}</span>}{task.estimateMinutes && <span>{task.estimateMinutes >= 60 ? `${Math.floor(task.estimateMinutes / 60)}h${task.estimateMinutes % 60 ? ` ${task.estimateMinutes % 60}min` : ""}` : `${task.estimateMinutes}min`}</span>}</div></div>
                    <span className={cn("rounded-md px-2 py-1 text-xs font-semibold", task.status === "completed" ? "bg-emerald-50 text-emerald-700" : task.status === "in_progress" ? "bg-[#eeeaff] text-[#6449d9]" : task.status === "waiting" ? "bg-amber-50 text-amber-700" : "bg-slate-100 text-slate-500")}>{STATUS_LABELS[task.status]}</span>
                  </article>
                )) : <div className="grid min-h-28 place-items-center px-4 text-center"><div><Clipboard className="mx-auto size-5 text-slate-300" /><p className="mt-2 text-xs font-medium text-slate-500">Nenhuma demanda vinculada</p><p className="mt-1 text-xs text-slate-400">Crie a primeira demanda para este cliente.</p></div></div>}
              </div>
        ...
        <aside className="space-y-4">
          ...
          {client.notes && <section className="rounded-xl border border-slate-200 bg-white p-5"><h2 className="text-sm font-semibold text-slate-900">Observações</h2><p className="mt-3 whitespace-pre-wrap text-xs leading-5 text-slate-500">{client.notes}</p></section>}
        </aside>
      </div>
    </div>
  );
...
function ContactRow({ icon: Icon, label, value, action }: { icon: typeof Mail; label: string; value: string; action?: React.ReactNode }) {
  return <div className="flex items-start gap-2.5"><Icon className="mt-0.5 size-3.5 shrink-0 text-slate-400" /><div className="min-w-0 flex-1"><p className="text-xs font-semibold uppercase tracking-wide text-slate-400">{label}</p><p className="mt-1 break-words text-xs leading-4 text-slate-600">{value || "Não informado"}</p></div>{action}</div>;
}
...
function FinancePanel({ client, compact = false }: { client: Client; compact?: boolean }) {
  return (
    <section className={cn("rounded-xl border border-slate-200 bg-white p-5", !compact && "mt-4")}>
      ...
      <div className="mt-4 rounded-lg bg-[#f4f5ff] p-3"><p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Valor contratual ativo</p><p className="mt-1 text-2xl font-semibold tracking-[-0.04em] text-slate-900">{currency.format(client.contractValue)} <span className="text-xs font-normal tracking-normal text-slate-400">{client.contractKind === "fixed" ? "/ mês fixo" : client.contractKind === "one_time" ? "/ projeto" : ""}</span></p></div>
      <div className="mt-2 grid grid-cols-2 gap-2"><div className="rounded-lg bg-slate-50 p-2.5"><p className="text-xs font-medium text-slate-400">Próximo vencimento</p><p className="mt-1 text-xs font-semibold text-slate-700">{client.nextDueDate ? format(parseISO(client.nextDueDate), "dd 'de' MMM", { locale: ptBR }) : "Não informado"}</p></div><div className="rounded-lg bg-slate-50 p-2.5"><p className="text-xs font-medium text-slate-400">Pagamento</p><p className={cn("mt-1 text-xs font-semibold", paymentStyles[client.paymentStatus])}>{PAYMENT_STATUS_LABELS[client.paymentStatus]}</p></div></div>
      <button type="button" onClick={() => toast.info("O histórico financeiro será conectado em uma próxima etapa.")} className="mt-3 h-8 w-full rounded-md bg-[#eff0ff] text-xs font-medium text-slate-600 transition hover:bg-[#e7e8ff]">Ver histórico de faturas</button>
    </section>
  );
}
