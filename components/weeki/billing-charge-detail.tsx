  ...
  return (
    <div className="mx-auto w-full max-w-[1500px] px-4 pb-24 pt-5 sm:px-6 lg:px-8">
      ...
      <section className="mt-5 rounded-xl bg-gradient-to-br from-[#0d1729] to-[#151c42] p-5 text-white sm:p-6">
        <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[9px] font-semibold uppercase tracking-[0.1em] text-white/45">Canal de recebimento digital</p><h2 className="mt-2 text-xl font-semibold tracking-[-0.035em] sm:text-[24px]">Link de pagamento pronto para envio</h2></div><span className={cn("inline-flex items-center gap-1.5 rounded-md border px-2.5 py-1.5 text-[9px] font-medium", gatewayConnected ? "border-emerald-300/20 bg-emerald-300/10 text-emerald-200" : "border-amber-300/20 bg-amber-300/10 text-amber-200")}><ShieldCheck className="size-3.5" />{gatewayConnected ? "Checkout conectado" : "Modo demonstrativo"}</span></div>
        <div className="mt-5 flex flex-col gap-2 sm:flex-row"><div className="flex h-10 min-w-0 flex-1 items-center gap-2 rounded-md bg-white px-3 text-[11px] text-slate-700"><Link2 className="size-3.5 shrink-0" /><span className="truncate">{publicLink}</span><span className="ml-auto hidden shrink-0 text-[8px] font-semibold uppercase text-slate-400 md:inline">{gatewayConnected ? "Ativo" : "Prévia"}</span></div><Button type="button" onClick={() => copyAndRegister("copied")} className="h-10 rounded-md bg-[#5947e5] px-4 text-[11px] shadow-none hover:bg-[#4b39d3]"><Copy className="size-3.5" /> Copiar link</Button></div>
        <div className="mt-4 flex flex-wrap items-center gap-2"><span className="mr-1 text-[10px] text-white/45">Compartilhar:</span><ShareButton icon={MessageCircle} label="WhatsApp" onClick={() => copyAndRegister("whatsapp")} /><ShareButton icon={Mail} label="E-mail" onClick={() => copyAndRegister("email")} /><ShareButton icon={QrCode} label="Exibir QR Code" onClick={() => toast.info("O QR Code estará disponível quando o checkout estiver conectado.")} /></div>
      </section>

      <div className="mt-4 grid items-start gap-4 xl:grid-cols-[minmax(0,1.25fr)_minmax(320px,0.75fr)]">
        <div className="space-y-4">
          <section className="rounded-xl border border-slate-200 bg-white p-4 sm:p-5">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-[9px] font-semibold uppercase tracking-[0.06em] text-slate-400">Fatura de serviços</p><h2 className="mt-2 max-w-xl text-base font-semibold leading-6 text-slate-900">{charge.description}</h2><p className="mt-1 text-[10px] text-slate-400">Identificador {charge.code}</p></div><div className="shrink-0 sm:text-right"><p className="text-[9px] uppercase text-slate-400">Valor total</p><p className="mt-1 text-[28px] font-bold tracking-[-0.045em] text-slate-900">{currency.format(charge.amount)}</p></div></div>
            {charge.discountEnabled && <div className="mt-4 flex flex-wrap items-center justify-between gap-2 rounded-lg bg-[#f2f1ff] px-3.5 py-3 text-[10px]"><span className="flex items-center gap-2 text-slate-600"><span className="grid size-7 place-items-center rounded-md bg-white text-[#5a45dc]"><Zap className="size-3.5" /></span>{BILLING_METHOD_LABELS[charge.discountMethod]} com {charge.discountPercent}% de desconto</span><strong className="text-xs font-semibold text-[#5843d9]">{currency.format(discountedAmount)}</strong></div>}
            ...
            <div className="mt-5"><p className="text-[9px] font-semibold uppercase tracking-[0.06em] text-slate-400">Tomador dos serviços</p><div className="mt-2 flex items-center gap-3 rounded-lg bg-slate-50 p-3.5"><span className="grid size-10 shrink-0 place-items-center rounded-lg bg-slate-900 text-xs font-semibold text-white">{client?.initials ?? "CL"}</span><div className="min-w-0"><p className="truncate text-xs font-semibold text-slate-900">{client?.name ?? "Cliente não encontrado"}</p><p className="mt-0.5 truncate text-[10px] text-slate-400">{client?.contactName || client?.segment || "Cadastro de cliente"}</p></div></div></div>
          </section>

          <section className="rounded-xl border border-slate-200 bg-white p-4 sm:p-5"><div className="flex items-center justify-between"><div><h2 className="text-sm font-semibold text-slate-900">Rastreabilidade e auditoria</h2><p className="mt-1 text-[10px] text-slate-400">Histórico de alterações e compartilhamentos.</p></div><span className="inline-flex items-center gap-1.5 text-[9px] font-medium text-[#5c46df]"><span className="size-1.5 rounded-full bg-[#654ff0]" /> Sincronizado</span></div><div className="mt-5 space-y-0">{charge.events.map((event, index) => <div key={event.id} className="relative flex gap-3 pb-5 last:pb-0"><span className={cn("relative z-10 mt-0.5 grid size-5 shrink-0 place-items-center rounded-full border-4 border-white", index === charge.events.length - 1 ? "bg-[#5f4aeb]" : "bg-slate-800")}><span className="size-1.5 rounded-full bg-white" /></span>{index < charge.events.length - 1 && <span className="absolute left-[9px] top-4 h-full w-px bg-slate-200" />}<div className="min-w-0 flex-1"><div className="flex flex-wrap items-center justify-between gap-1"><p className="text-[11px] font-medium text-slate-800">{event.title}</p><time className="text-[9px] tabular-nums text-slate-400">{format(parseISO(event.createdAt), "dd MMM, HH:mm", { locale: ptBR })}</time></div><p className="mt-0.5 text-[10px] leading-4 text-slate-500">{event.description}</p></div></div>)}</div></section>
        </div>

        <aside className="space-y-4">
          <section className="rounded-xl border border-slate-200 bg-white p-4"><div className="flex items-center justify-between"><h2 className="text-sm font-semibold text-slate-900">Métodos de pagamento</h2><span className="text-[9px] text-slate-400">{charge.methods.length} habilitados</span></div><div className="mt-4 space-y-2">{(["pix", "credit_card", "bank_slip"] as BillingMethod[]).map((method) => <PaymentMethodRow key={method} method={method} active={charge.methods.includes(method)} installments={charge.cardMaxInstallments} />)}</div></section>
...
function ShareButton({ icon: Icon, label, onClick }: { icon: typeof Mail; label: string; onClick: () => void }) { return <button type="button" onClick={onClick} className="flex h-8 items-center gap-2 rounded-md bg-white/10 px-3 text-[10px] font-medium text-white/80 transition hover:bg-white/15"><Icon className="size-3.5" />{label}</button>; }
function InfoTile({ icon: Icon, label, value, detail }: { icon: typeof CalendarDays; label: string; value: string; detail: string }) { return <div className="rounded-lg bg-[#f1f3ff] p-3"><p className="flex items-center gap-1.5 text-[8px] font-semibold uppercase tracking-[0.05em] text-slate-500"><Icon className="size-3 text-[#604ae3]" />{label}</p><p className="mt-2 text-xs font-semibold text-slate-900">{value}</p><p className="mt-1 text-[9px] text-[#5d47df]">{detail}</p></div>; }
function PaymentMethodRow({ method, active, installments }: { method: BillingMethod; active: boolean; installments: number }) { const Icon = method === "pix" ? Zap : method === "credit_card" ? CreditCard : Barcode; return <div className={cn("flex items-center gap-3 rounded-lg p-3", active ? "bg-[#f0f1ff]" : "bg-slate-50 opacity-55")}><span className={cn("grid size-8 place-items-center rounded-md", active ? "bg-slate-900 text-white" : "bg-slate-200 text-slate-400")}><Icon className="size-4" /></span><div className="min-w-0 flex-1"><p className="text-[11px] font-medium text-slate-800">{BILLING_METHOD_LABELS[method]}</p><p className="mt-0.5 text-[9px] text-slate-400">{method === "credit_card" ? `Até ${installments}x` : method === "pix" ? "Confirmação rápida" : "Compensação bancária"}</p></div><span className={cn("text-[9px] font-medium", active ? "text-[#5943dc]" : "text-slate-400")}>{active ? "Ativo" : "Inativo"}</span></div>; }
function ActionRow({ icon: Icon, title, detail, onClick }: { icon: typeof Mail; title: string; detail: string; onClick: () => void }) { return <button type="button" onClick={onClick} className="flex w-full items-center gap-3 py-3 text-left"><span className="grid size-8 shrink-0 place-items-center rounded-md bg-[#f0efff] text-[#5b45dc]"><Icon className="size-4" /></span><span className="min-w-0 flex-1"><span className="block text-[11px] font-medium text-slate-800">{title}</span><span className="mt-0.5 block truncate text-[9px] text-slate-400">{detail}</span></span><span className="text-slate-300">›</span></button>; }
