...
function localPasswordHash(password: string) {
  return password.trim()
    ? reportSnapshotHash({ password: password.trim() })
    : null;
}

function versionFromReport(
  report: Pick<
    WeekiReport,
    "sourceSnapshot" | "blocks" | "metrics" | "versions"
  >,
  reason: string,
  immutable: boolean,
): ReportVersion {
...
function normalizeReport(report: WeekiReport): WeekiReport {
  ...
  return {
    ...
    selectedCategories: Array.isArray(report.selectedCategories)
      ? report.selectedCategories
      : [],
    sourceSnapshot: {
      ...report.sourceSnapshot,
      activities: Array.isArray(report.sourceSnapshot?.activities)
        ? report.sourceSnapshot.activities
        : [],
      financial: {
        charges: Array.isArray(report.sourceSnapshot?.financial?.charges)
          ? report.sourceSnapshot.financial.charges
          : [],
        transactions: Array.isArray(
          report.sourceSnapshot?.financial?.transactions,
        )
          ? report.sourceSnapshot.financial.transactions
          : [],
      },
    ...
    emailDispatches: Array.isArray(report.emailDispatches)
      ? report.emailDispatches
      : [],
    clientDecisions: Array.isArray(report.clientDecisions)
      ? report.clientDecisions
      : [],
...
function nextReportNumber(current: WeekiReport[]) {
  ...
  const count =
    current.filter((report) => report.number.includes(`-${year}-`)).length + 1;
...
export function useWeekiReports() {
  ...
  const createReport = useCallback(
    (draft: ReportDraftInput) => {
      ...
      const first: WeekiReport = normalizeReport({
        ...
        events: [
          eventFor(
            "created",
            "Relatório criado",
            `Estrutura inicial criada para ${draft.clientName || "cliente não selecionado"}.`,
          ),
        ],
        finalizedAt: null,
        sentAt: null,
      ...
      const initialVersion = versionFromReport(
        first,
        "Relatório criado como rascunho",
        false,
      );
      const report = {
        ...first,
  ...
  const updateReport = useCallback(
    (id: string, update: Partial<WeekiReport>, recordEvent = true) => {
            ...
            events: recordEvent
              ? [
                  eventFor(
                    ...
                    "As informações do relatório foram revisadas.",
                  ),
                  ...report.events,
                ]
              : report.events,
  ...
  const createVersion = useCallback(
    (id: string, reason: string, immutable = false) => {
      ...
      setReports((current) =>
        current.map((report) => {
          ...
          return normalizeReport({
            ...
            status:
              immutable && report.status === "draft" ? "ready" : report.status,
            finalizedAt: immutable
              ? (report.finalizedAt ?? new Date().toISOString())
              : report.finalizedAt,
            updatedAt: new Date().toISOString(),
            events: [
              eventFor(
                immutable ? "finalized" : "version_created",
                reason,
                immutable
                  ? "Snapshot preservado para envio ou compartilhamento."
                  : "Uma nova versão editável foi registrada.",
              ),
              ...report.events,
            ],
          });
  ...
  const finalizeReport = useCallback(
    (id: string, update?: Partial<WeekiReport>) => {
      let saved: WeekiReport | null = null;
      setReports((current) =>
        current.map((report) => {
          if (report.id !== id) return report;
          ...
          saved = normalizeReport({
            ...
            status:
              ...
              base.status === "adjustment_requested"
                ? base.status
                : "ready",
            currentVersionId: next.id,
            ...
            events: [
              eventFor(
                ...
                "Os dados desta versão foram preservados.",
              ),
              ...base.events,
            ],
          });
          ...
          ? normalizeReport({
              ...
              events: [
                eventFor(
                  "archived",
                  "Relatório arquivado",
                  "O histórico e versões foram preservados.",
                ),
                ...report.events,
              ],
            })
    ...
    setReports((current) =>
      current.map((report) =>
        report.id === id
          ? normalizeReport({
              ...report,
              status: report.finalizedAt ? "ready" : "in_progress",
              archivedAt: null,
              updatedAt: new Date().toISOString(),
              events: [
                eventFor(
                  ...
                  "O relatório voltou para a lista ativa.",
                ),
                ...report.events,
              ],
            })
  ...
  const deleteDraft = useCallback((id: string) => {
    setReports((current) =>
      current.filter((report) => report.id !== id || report.status !== "draft"),
    );
  }, []);
          ...
          return normalizeReport({
            ...
            events: [
              eventFor(
                ...
                "Token público não sequencial preparado para este relatório.",
              ),
              ...report.events,
            ],
          });
          ...
          ? normalizeReport({
              ...
              events: [
                eventFor(
                  "share_revoked",
                  "Link revogado",
                  "O acesso público foi encerrado para este token.",
                ),
                ...report.events,
              ],
            })
  ...
  const prepareEmail = useCallback(
    (
      id: string,
      input: Omit<ReportEmailDispatch, "id" | "status" | "createdAt">,
    ) => {
          ...
          return normalizeReport({
            ...
            events: [
              eventFor(
                ...
                `Template aberto para ${input.recipient}.`,
              ),
              ...report.events,
            ],
          });
          ...
          ? normalizeReport({
              ...
              events: [
                eventFor(
                  "pdf_generated",
                  "PDF gerado",
                  "Uma cópia em PDF foi baixada localmente.",
                ),
                ...report.events,
              ],
            })
  ...
  const findByShareToken = useCallback(
    (token: string) =>
      reports.find((report) => report.share?.token === token) ?? null,
    [reports],
  );

  const checkSharePassword = useCallback(
    ...
    setReports((current) =>
      current.map((report) => {
        ...
        const expired = Boolean(
          report.share.expiresAt && report.share.expiresAt < now.slice(0, 10),
        );
        ...
        saved = normalizeReport({
          ...
          status:
            ...
            report.status === "adjustment_requested"
              ? report.status
              : "viewed",
          share: {
            ...report.share,
          ...
          events: [
            eventFor(
              "share_opened",
              "Link aberto",
              "O relatório foi visualizado pelo link compartilhável.",
              "Cliente",
            ),
            ...report.events,
          ],
  ...
  const registerClientDecision = useCallback(
    (
      token: string,
            ...
            events: [
              eventFor(
                ...
                kind === "approved"
                  ? "Relatório aprovado"
                  : "Ajuste solicitado",
                decision.message || "Sem observação adicional.",
                "Cliente",
              ),
              ...report.events,
            ],
  ...
  const activeReports = useMemo(
    () => reports.filter((report) => !report.archivedAt),
    [reports],
  );
