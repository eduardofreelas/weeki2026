...
function EventOption({ eventType, selected, onSelect }: { eventType: EventType; selected: boolean; onSelect: () => void }) {
  return <button type="button" onClick={onSelect} className={cn("rounded-xl border border-slate-200 p-4 text-left transition hover:border-[#aaa0e8]", selected && "border-[#6a54e8] bg-[#faf9ff] ring-1 ring-[#6a54e8]/10")}><div className="flex items-start justify-between gap-3"><h2 className="text-sm font-semibold text-slate-900">{eventType.name}</h2><span className={cn("mt-0.5 grid size-4 shrink-0 place-items-center rounded-full border", selected ? "border-[#654ce4] bg-[#654ce4] text-white" : "border-slate-300")}><Check className="size-2.5" /></span></div><p className="mt-1.5 line-clamp-2 text-xs leading-4 text-slate-500">{eventType.description}</p><div className="mt-3 flex items-center gap-3 text-xs text-slate-500"><span className="flex items-center gap-1"><Clock3 className="size-3" />{formatDuration(eventType.durationMinutes)}</span><span className="flex items-center gap-1"><ModeIcon mode={eventType.mode} />{APPOINTMENT_MODE_LABELS[eventType.mode]}</span></div></button>;
}

function StepTitle({ number, title }: { number: string; title: string }) { return <div className="flex items-center gap-2"><span className="grid size-5 place-items-center rounded-md bg-[#ede9ff] text-xs font-semibold text-[#5c43d8]">{number}</span><h2 className="text-xs font-semibold text-slate-800">{title}</h2></div>; }
