      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
          <form onSubmit={submit} className="rounded-2xl border border-slate-200 bg-white p-5 sm:p-7">
            ...
            <section className="mt-7"><StepTitle number="2" title="Data e horário" /><div className="mt-3 max-w-[240px]"><Label className="mb-1.5 block text-[10px] font-semibold uppercase text-slate-500">Data</Label><Input type="date" min={minimumDate} max={maximumDate} value={date} onChange={(event) => { setDate(event.target.value); setTime(""); }} className="h-9 rounded-md text-xs shadow-none" /></div><div className="mt-3 grid grid-cols-3 gap-2 sm:grid-cols-6">{times.map((slot) => <button key={slot} type="button" disabled={!selectedType || unavailableTimes.has(slot) || date < minimumDate || Boolean(maximumDate && date > maximumDate)} onClick={() => setTime(slot)} className={cn("h-9 rounded-md border border-slate-200 bg-white text-xs font-medium text-slate-600 transition hover:border-[#9584e8] disabled:cursor-not-allowed disabled:bg-slate-50 disabled:text-slate-300", time === slot && "border-[#654ce4] bg-[#f2efff] text-[#5941d6]")}>{slot}</button>)}</div></section>
          ...
          <aside className="h-fit overflow-hidden rounded-2xl border border-slate-200 bg-white lg:sticky lg:top-8">{selectedType?.coverImage && <div className="h-32 bg-cover bg-center" style={{ backgroundImage: `url(${selectedType.coverImage})` }} />}<div className="p-5"><p className="text-[10px] font-semibold uppercase tracking-[0.06em] text-slate-400">Resumo</p>{selectedType ? <><h2 className="mt-4 text-base font-semibold text-slate-900">{selectedType.name}</h2><p className="mt-1 text-xs leading-5 text-slate-500">{stripHtml(selectedType.description)}</p><div className="mt-4 space-y-2 border-t border-slate-100 pt-4 text-xs text-slate-600"><p className="flex items-center gap-2"><Clock3 className="size-3.5 text-slate-400" />{formatDuration(selectedType.durationMinutes)}</p><p className="flex items-center gap-2"><ModeIcon mode={selectedType.mode} />{APPOINTMENT_MODE_LABELS[selectedType.mode]}</p>{selectedType.mode === "in_person" && selectedType.location && <p className="flex items-center gap-2"><MapPin className="size-3.5 text-slate-400" />{selectedType.location}</p>}{date && time && <p className="flex items-center gap-2"><CalendarDays className="size-3.5 text-slate-400" />{date.split("-").reverse().join("/")} às {time}</p>}</div><p className="mt-5 rounded-md bg-[#f3f1ff] px-3 py-2 text-[11px] text-[#5946d4]">{autoApproval ? "Confirmação imediata após o envio." : "O horário será reservado após aprovação."}</p></> : <div className="py-12 text-center"><CalendarDays className="mx-auto size-6 text-slate-300" /><p className="mt-3 text-xs text-slate-400">Selecione um atendimento para ver os detalhes.</p></div>}</div></aside>
        </div>
      </div>
    </main>
  );
