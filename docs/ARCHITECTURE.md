...
O domínio de Orçamentos substitui Serviços como módulo principal da navegação comercial. A V1 usa `features/operations/` para preservar atendimentos, oportunidades e o catálogo interno de itens salvos; a tela `components/weeki/quotes-screen.tsx` cria propostas, gera PDF/Excel, prepara links, registra aceite comercial e converte orçamentos aprovados em cobrança, contrato ou atendimento. A migration `006_quotes.sql` prepara o schema server-only `weeki_quotes` com orçamentos, itens, versões, eventos, tokens públicos, modelos, configurações e itens salvos.

Publicar somente `out/` mantém o protótipo estático, mas deixa persistência pública real de orçamentos, pagamentos conectados, emissão fiscal real, IA de contratos, IA de relatórios e assinatura eletrônica indisponíveis. A implantação completa serve frontend e APIs na mesma origem por `npm run start:payments`. Consulte [payments.md](payments.md), [fiscal.md](fiscal.md), [contracts.md](contracts.md), [reports.md](reports.md) e [quotes.md](quotes.md) para topologia, segurança, migrations e homologação.
